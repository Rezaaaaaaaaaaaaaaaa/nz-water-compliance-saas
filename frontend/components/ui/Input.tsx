...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const inputId = id || props.name || (label ? `input-${label.toLowerCase().replace(/\s+/g, '-')}` : generatedId);
...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    ...
    // Use React's useId for stable, hydration-safe IDs
    const generatedId = React.useId();
    const textareaId = id || props.name || (label ? `textarea-${label.toLowerCase().replace(/\s+/g, '-')}` : generatedId);
